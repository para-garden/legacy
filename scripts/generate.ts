// Usage: bun run scripts/generate.ts --character <name> [--prompt <file>] [--load <file>] [--save <file>]
// Reads character card from characters/<name>.md (strips frontmatter)
// Default prompt from prompts/<name>.txt, overridden by --prompt
// Saves session JSON to logs/<name>/<timestamp>.json
...
const cardPath = `characters/${character}.md`;
if (!existsSync(cardPath)) {
  console.error(`Character card not found: ${cardPath}`);
  process.exit(1);
}
const cardRaw = readFileSync(cardPath, "utf8");
const SYSTEM = cardRaw.replace(/^---[\s\S]*?---\n/, "").trim();

const promptFile = args.prompt ?? `prompts/${character}.txt`;
